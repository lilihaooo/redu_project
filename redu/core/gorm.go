...
func InitGorm() {
	...
	gormLogPath := root + "/log/gorm.log"

	// 打开一个文件，用于记录慢查询日志
	gormLogFile, err = os.OpenFile(gormLogPath, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0666)
	if err != nil {
	...
	if level == "info" {
		...
		conf = logger.Config{
			...
			SlowThreshold: 1000 * time.Millisecond, // 查询超过1秒钟将被视为慢查询
