...
// 解析url
func urlInit(page int, categoryID string) string {
	// 获取当前时间
...
func grab(wg *sync.WaitGroup, categoryID string) {
	// 直接抓取0到1000页, 抓取失败终止循环
	for i := 0; i <= 1000; i++ {
		urlStr := urlInit(i, categoryID)
		jsonData := ""
		for j := 0; j < 5; j++ {
